...
func CallbackHandler(c *gin.Context) {
	...
	if pot_user.UserID == 0 {
		if database.InsertUser(db,
			...
			// Avatar to db
			avatar_url, err := database.GetAvatarByUsername(db, string(userResponse.Response[0].FirstName+" "+userResponse.Response[0].LastName))
			if avatar_url != "" {
				err = database.UpdateAvatar(db, vk_user.UserID, "static/images/"+string(userResponse.Response[0].FirstName)+" "+string(userResponse.Response[0].LastName)+".jpeg")
				if err != nil {
					log.Fatal(err)
				}
				c.HTML(
					...
					gin.H{
						...
						"id":           vk_user.UserID,
						"balance":      wallet,
						"avatar_url":   "static/images/" + string(userResponse.Response[0].FirstName) + " " + string(userResponse.Response[0].LastName) + ".jpeg",
					},
				)
				return
			} else {
				err = database.CreateAvatar(db, vk_user.UserID, "static/images/"+string(userResponse.Response[0].FirstName)+" "+string(userResponse.Response[0].LastName)+".jpeg")
				if err != nil {
					log.Fatal(err)
				}
				c.HTML(
